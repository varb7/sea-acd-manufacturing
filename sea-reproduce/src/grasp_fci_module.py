#!/usr/bin/env python3
"""
PyTetrad GRaSP-FCI wrapper (hybrid causal discovery, outputs PAG).

GRaSP-FCI is a hybrid algorithm that uses:
- GRaSP (Greedy Relaxations of Sparsest Permutation) for initial ordering
...
class TetradGraspFci:
    """
    GRaSP-FCI wrapper using PyTetrad.
    ...
    def run(self, data: Union[pd.DataFrame, np.ndarray], columns: Optional[list] = None,
        ...
        """
        Run GRaSP-FCI and return FCI-compatible adjacency matrix.
        ...
        try:
            alg = self.search.GraspFci(indep, sc)
        except AttributeError:
            # Try alternative class name if GraspFci doesn't exist
            try:
                alg = self.search.Grasp(sc)
                print("[WARNING] GraspFci not found, falling back to Grasp algorithm")
            except AttributeError:
                raise RuntimeError("GRaSP-FCI algorithm not available in this Tetrad version")
...
def run_grasp_fci(
    ...
    """
    Convenience function to run GRaSP-FCI algorithm.
...
if __name__ == "__main__":
    print("Tetrad GRaSP-FCI Module (sanity test)")
    np.random.seed(42)
    n = 1000
    ...
    print("Running GRaSP-FCI (alpha=0.05, depth=2)…")
    adj = run_grasp_fci(df, alpha=0.05, depth=2)
