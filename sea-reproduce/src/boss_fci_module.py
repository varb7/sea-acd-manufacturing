#!/usr/bin/env python3
"""
PyTetrad BOSS-FCI wrapper (hybrid causal discovery, outputs PAG).

BOSS-FCI is a hybrid algorithm that uses:
- BOSS (Best Order Score Search) for initial ordering
- FCI orientation rules for final PAG construction
...
class TetradBossFci:
    ...
    def _create_score_function(self, tetrad_data, cats, cont):
        """Create score function by data type."""
        if cats and cont:
            sc = self.score.ConditionalGaussianBicScore(tetrad_data)
            sc.setDiscretize(True)
            sc.setNumCategoriesToDiscretize(3)
        elif cats and not cont:
            sc = self.score.BDeuScore(tetrad_data)
            sc.setPriorEquivalentSampleSize(10.0)
        else:
    ...
    def run(self, data: Union[pd.DataFrame, np.ndarray], columns: Optional[list] = None,
        ...
        """
        Run BOSS-FCI and return FCI-compatible adjacency matrix.
        ...
        try:
            alg = self.search.BossFci(indep, sc)
        except AttributeError:
            # Try alternative class name if BossFci doesn't exist
            try:
                alg = self.search.Boss(sc)
                # If Boss exists but not BossFci, run Boss followed by FCI rules
                print("[WARNING] BossFci not found, falling back to Boss algorithm")
            except AttributeError:
                raise RuntimeError("BOSS-FCI algorithm not available in this Tetrad version")
...
def run_boss_fci(
    ...
    """
    Convenience function to run BOSS-FCI algorithm.
...
if __name__ == "__main__":
    print("Tetrad BOSS-FCI Module (sanity test)")
    np.random.seed(42)
    n = 1000
    ...
    print("Running BOSS-FCI (alpha=0.05, depth=2)…")
    adj = run_boss_fci(df, alpha=0.05, depth=2)
