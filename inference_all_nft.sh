...
# Excludes scripts ending with _ft.sh
...
# Find all inference scripts, excluding *_ft.sh
INFERENCE_SCRIPTS=$(find "$SCRIPT_DIR" -name "inference_*.sh" ! -name "inference_*_ft.sh" | sort)

if [ -z "$INFERENCE_SCRIPTS" ]; then
    echo "No inference_*.sh files found in $SCRIPT_DIR (excluding *_ft.sh)"
    exit 1
fi

# Count scripts
TOTAL=$(echo "$INFERENCE_SCRIPTS" | wc -l)
echo "Found $TOTAL inference scripts (excluding *_ft.sh)"
echo ""
